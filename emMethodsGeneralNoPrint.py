...
def generateNoiseCipher(ciphertext):
  #randomly permute two pairs of cipher letters
  ...
  while position3 != position1 and position3 != position2:
    position3 = random.randint(0,len(ciphertext)-1)
  position4 = random.randint(0,len(ciphertext)-1)
  while position4 != position3 and position4 != position1 and position4 != position2:
    position4 = random.randint(0,len(ciphertext)-1)
...
def calcAccuracy(gold_data,output_data) :
  if not len(output_data) == len(gold_data) :
    print 'The gold file is of length %d and the output file is of length %d'%(len(gold_data),len(output_data))
    sys.exit()
  ...
  for i in range(0,len(gold_data)) :
    total_items += len(gold_data[i])
    for j in range(0, len(gold_data[i])) :
      if gold_data[i][j] == output_data[i][j]:
        matches += 1
