...
def main():
  ...
  parser.add_option("--order", action="store", type="int", dest="order",default = 3,help="the order of the lm")
  parser.add_option("--word_unigrams", action="store", type="string", dest="word_unigrams",default="lm.arpa",help="The word unigram probs file")
  
  (options, args) = parser.parse_args()
  arpa_file = options.arpa_file
  carmel_file = options.carmel_file
  order = options.order
  word_unigrams = options.word_unigrams
  ...
  word_probs = defaultdict(str)
  for line in open(word_unigrams):
    line = line.strip()
  ...
  writeFSA(carmel_file,ngrams_f,ngrams_bow,order,word_probs)
def writeFSA(carmel_file,ngrams_f,ngrams_bow,order,word_probs):
  output_file = open(carmel_file,'w')
  output_file.write("END\n")
  output_file.write("(0 (WORD_GRAM_START *e* *e* 0.05!))\n")
  output_file.write("(0 (CHAR_GRAM_START *e* *e* 0.95!))\n")
  ...
  #writing the word ngrams
  for word in word_probs:
    no_space_word = ''.join(word.split())
    output_file.write("(WORD_GRAM_START (%s *e* *e* %s!))\n"%(no_space_word,word_probs[word]))
    letters = word.split()
    for i,letter in enumerate(letters):
      if i == 0:
        output_file.write("(%s (%s-0 *e* \"%s\" 1.0!))\n"%(no_space_word,no_space_word,letter))
      else :
        output_file.write("(%s-%d (%s-%d *e* \"%s\" 1.0!))\n"%(no_space_word,i-1,no_space_word,i,letter))
    output_file.write("(%s-%d (WORD_GRAM_END *e* *e* 1.0!))\n"%(no_space_word,len(letters)-1))

  output_file.write("(WORD_GRAM_END (0 *e* \"_\" 1.!))\n")
