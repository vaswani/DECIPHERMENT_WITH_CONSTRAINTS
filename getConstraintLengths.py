...
def inip():
	...
	global probabilities_language, probabilities_channel,initial_parameter_args,init_option,current_optimization_tag
...
#   Number of constraints (equalities plus inequalities)
	...
	m = 1 #the number of constraints will only be 1
#   Initial point
	...
	if init_option == 'current_prob' : #then we have to initize it 
		...
		for next_tag in initial_parameter_args[current_optimization_tag].keys() :
			if initial_parameter_args[current_optimization_tag][next_tag] ==  'ONE' :
				...
				x[parameter_number] = probabilities_language[current_optimization_tag][next_tag]	
	else :
...
def evalf(x):
	...
	global initial_parameter_args,current_fractional_counts,sigma,alpha,current_optimization_tag
	...
	for next_tag in initial_parameter_args[current_optimization_tag].keys() :
		if initial_parameter_args[current_optimization_tag][next_tag] == 'ONE' : #this ie never going to happen
			break
		parameter_number = initial_parameter_args[current_optimization_tag][next_tag]
		if x[parameter_number] == float(0) :
			#flag = -1
			#print 'the flag became negavie'
			func_value += current_fractional_counts[current_optimization_tag][next_tag]  * -little_infinity + alpha * math.exp(-x[parameter_number]/sigma)
		else :
...
def evalg(x):
	...
	global initial_parameter_args,current_fractional_counts,alpha,sigma,parameter_counter,num_constraints,current_optimization_tag
	...
	for next_tag in initial_parameter_args[current_optimization_tag].keys() :
		...
		if x[parameter_number] == float(0) :
			...
			#flag = -1
				#break
			g[parameter_number]  = -1*(current_fractional_counts[current_optimization_tag][next_tag] * little_infinity - alpha*1/sigma)
		else :
			g[parameter_number]  = -1*(current_fractional_counts[current_optimization_tag][next_tag] /x[parameter_number] - alpha*math.exp(-x[parameter_number]/sigma)/sigma)
	#if flag== -1 :
	#	break
	#n = len(x)
...
def evalh(x):
	global current_fractional_counts,initial_parameter_args,alpha,sigma,current_optimization_tag
	...
	counter = 0
	for next_tag in initial_parameter_args[current_optimization_tag].keys() :
		if initial_parameter_args[current_optimization_tag][next_tag] == 'ONE' : #this again wont happen
			continue
		else :
			parameter_number = initial_parameter_args[current_optimization_tag][next_tag]
			hessian_term = current_fractional_counts[current_optimization_tag][next_tag] / (x[parameter_number]*x[parameter_number]) - alpha * math.exp(-x[parameter_number]/sigma)/(sigma*sigma)
...
def evalc(x,ind):
	global constraint_tags_dict, current_fractional_counts,initial_parameter_args,num_constraints,current_optimization_tag
	...
	tag_corresponding_to_constraint = current_optimization_tag #constraint_tags_dict[ind]
...
def evaljac(x,ind):
	global initial_parameter_args,constraint_tags_dict,current_optimization_tag
	...
	'''
	flag  = 0
	constraint_tag = current_optimization_tag #constraint_tags_dict[ind] #getting the tag corresponding to the constraint id
...
def evalhc(x,ind):
	global num_constraints,current_optimization_tag
	#print 'the constraint number is %d'%ind
